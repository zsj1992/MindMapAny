...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd = [
    ...
    { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: '首页', item: SITE_URL }, { '@type': 'ListItem', position: 2, name: '博客', item: `${SITE_URL}/blog` }, { '@type': 'ListItem', position: 3, name: post.title, item: url }] },
  ];
  return (
    <>
      <JsonLd data={jsonLd} />
      <main>
        <article className="mx-auto max-w-4xl px-5 py-12 lg:px-8 lg:py-16">
          <Breadcrumbs items={[{ label: '首页', href: '/' }, { label: '博客', href: '/blog' }, { label: post.title }]} />
          <header className="mt-10 border-b pb-10" style={{ borderColor: 'var(--border)' }}><div className="text-xs font-bold text-brand-600">{post.category}</div><h1 className="mt-4 text-4xl font-bold leading-tight tracking-[-0.04em] sm:text-5xl">{post.title}</h1><p className="mt-5 text-lg leading-8 text-text-muted">{post.description}</p><div className="mt-6 flex flex-wrap gap-3 text-xs text-text-subtle"><span>{post.author}</span><span>·</span><time dateTime={post.publishedAt}>{post.publishedAt}</time><span>·</span><span>{post.readingMinutes} 分钟阅读</span></div></header>
          <div className="mt-10 grid gap-10 lg:grid-cols-[180px_1fr]">
            <aside><div className="sticky top-24 rounded-xl border bg-bg-subtle p-4" style={{ borderColor: 'var(--border)' }}><div className="text-xs font-bold">目录</div><ol className="mt-3 space-y-2">{post.sections.map((section) => <li key={section.id}><Link href={`#${section.id}`} className="text-xs leading-5 text-text-muted hover:text-brand-600">{section.title}</Link></li>)}</ol></div></aside>
          ...
          <div className="mt-14 rounded-2xl bg-[#102f53] px-6 py-10 text-center text-white"><h2 className="text-2xl font-bold">把方法用到你的内容上</h2><p className="mt-3 text-sm text-blue-100/80">从真实文本、PDF、网页或视频生成一张可编辑脑图。</p><Link href={post.relatedTool} className="btn mt-6 h-11 bg-white px-6 text-[#102f53]">打开相关工具 →</Link></div>
