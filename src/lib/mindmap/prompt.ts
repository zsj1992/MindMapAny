...
export function buildSystemPrompt(opts: { language: string; depth: Depth; purpose: Purpose }): string {
  ...
  return [
    ...
    '4. 【叶子节点写成「标签：说明」】标签是 2-8 字的名词短语，冒号后是一句完整、能独立读懂的说明。',
    '   示例：好 → "噪音限制：23:00 至 08:00 为安静时段，考试期间 24 小时安静"',
    '   示例：差 → "噪音限制" / "23:00" / "安静时段"（拆成三个碎节点，读者要自己拼）',
    '5. 【宁可少而密，不要多而稀】能写进一句说明的内容不要拆成多个节点。',
    '   非叶子节点只写标签，不写说明，因为它的信息由子节点承载。',
    '6. 非叶子节点标题 2-10 字；叶子节点连标签带说明不超过 60 字。',
    '7. 根标题不超过 20 字，是内容的名字而不是概括，不要写成完整句子。',
    '8. 每个叶子节点必须以 ^chunkId 结尾。只能用下方给出的 chunkId，禁止编造。',
    '9. 禁止输出页码、时间戳、章节号 —— 这些由系统根据 chunkId 自动还原。',
    '10. 同一父节点下不要出现语义重复的兄弟节点；宁可合并，也不要凑数。',
    '11. 忠于原文，不补充原文没有的信息，不做评价。',
    '',
