...
export const ALL_TOOL_PAGES: ToolPage[] = [
  ...
  },
  {
    ...
    description:
      'Paste a video link. We read its captions and lay out the argument as a map where every node carries the timestamp it came from — click one and the video opens at that second. It works on lectures, conference talks, tutorials and reviews: anything where the substance is spoken rather than written down. Videos without captions cannot be mapped, and the tool says so plainly rather than returning something built on nothing.',
    ...
    benefits: [
      { title: 'A YouTube to mind map tool that shows its working', description: 'Timestamps are not decoration. Click one and the video opens at that second, so you verify against the recording itself rather than trusting a summary. Captions can mistranscribe a term; watching the moment catches it immediately, which is the same reason our document maps carry page numbers.' },
      { title: 'An hour of talk, laid out flat', description: 'Speech is linear and slow — roughly 150 words a minute, with no headings and no way to skim. A map gives spoken material the structure written material already has, so an hour of talk becomes something you can take in at a glance and return to selectively.' },
      { title: 'Find the part you actually need', description: 'Most of the value in a long video is concentrated in a few minutes, and finding those minutes normally means scrubbing the progress bar and guessing. Read the map, pick the branch that matters, click its timestamp.' },
      { title: 'Editable, not a dead export', description: 'The result is a working map, not a screenshot. Rename nodes, regroup branches, or describe the change in plain language — "merge these two", "expand the methodology" — and the timestamps survive the rewrite. Export to PNG, SVG or Markdown, or publish a link.' },
      { title: 'Captions in one language, map in another', description: 'If the captions are not in the language you asked for, the map is produced in your language and the notes record that it was translated. A Japanese talk can become a Chinese map without you doing anything, and without hiding that a translation happened.' },
    ],
    steps: [
      { title: 'Paste the video link', description: 'A normal watch link, a youtu.be short link or a Shorts URL all work. The link is checked before anything else runs, so an unrecognised URL is rejected immediately instead of after a wait.' },
      { title: 'Captions are read and grouped', description: 'A single caption cue is only a few words, which is too little to reason about. Cues are merged into roughly thirty-second passages so each one carries a complete thought, and every passage keeps the time it started.' },
      { title: 'The map is built with those times attached', description: 'Topics and sub-topics are derived from the passages, and each node inherits the timestamp of the passage it came from. Nothing invented by the model gets a timestamp — a node with no time attached is telling you it was not traced to a specific moment.' },
      { title: 'Check by clicking a timestamp', description: 'Any node you doubt takes one click to verify against the video itself. This is the step that makes the rest trustworthy.' },
    ],
    useCases: [
      'Lectures and course videos: turn a 90-minute recording into a structure you can revise from, then click back into the exact moment a concept was introduced rather than scrubbing for it',
      'Conference talks and interviews: separate the speaker\'s argument from the anecdotes, and keep the timestamp of each claim so quoting it later takes one click',
      'Product reviews and comparisons: pull the actual criteria out of a twenty-minute review, so you can see what was measured instead of rewatching for the verdict',
      'Tutorials and walkthroughs: get the sequence of steps as a list you can follow at your own pace, with the video moment attached to any step that needs watching',
      'Deciding what to watch: read the map first and find out in thirty seconds whether an hour-long video covers what you need',
      'Research and literature reviews: map recorded talks alongside papers, so video sources sit in the same structure as written ones',
    ],
    faq: [
