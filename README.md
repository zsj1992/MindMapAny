...
把长内容变成结构清晰、可溯源的脑图。文本 / PDF / DOCX / EPUB / PPTX / 网页 → 可编辑的思维导图。

## 架构

一条通用管线，多种输入共用；深度研究在上游增加“检索 + 多来源报告”阶段：

```
输入 → 提取(Block[]) → 切块(chunkId ↔ 页码/章节/幻灯片) → LLM 产 Markdown 大纲 → 确定性解析成 JSON → 渲染/编辑/导出
...
| `src/lib/extract/` | PDF / DOCX / EPUB / PPTX / 纯文本 / 网页提取，含 SSRF 与压缩包安全防护 |
...
运行时密钥通过 `wrangler secret` 配置，数据库使用 `wrangler.jsonc` 中绑定的 D1。

## 当前边界

已实现：文本 / Markdown / PDF / DOCX / EPUB / PPTX / 网页、带引用的多来源深度研究、编辑、折叠、PNG/SVG/Markdown 导出、保存、公开分享页（SSR + OG + sitemap）、积分与套餐闸门。

不支持：扫描件 OCR、旧版 DOC/PPT、Excel、音视频与 YouTube 转录、纯 JS 渲染页面、需登录页面、协作。
